...
That's annoying in a normal backend. In AI pipelines, it's expensive. Every rerun means buying the same tokens you already paid for. And when you're iterating on prompts and the only thing that changed is the last step, rerunning five upstream LLM calls that haven't changed is just throwing money away.
...
That's DriftQ. One binary. One process. No external dependencies. WAL-backed durability. A built-in dashboard. You can run it on a $5/month VPS.

## The actual differences, no fluff

Here's how I think about it.

Temporal is a **durable execution platform**. You adopt its programming model. You write deterministic workflow code, separate your side effects into activities, deploy a multi-service cluster with a persistence database, and Temporal's event-sourced history drives everything forward. That's powerful. But it's a commitment, you're buying into an architecture.
...
With DriftQ's v2 replay, you can re-drive from the step that actually changed and reuse the intermediate outputs from earlier steps. In a simple example I walked through in a previous post, that's roughly 66% cheaper for the exact same workflow on the exact same model. The model didn't get cheaper. The workflow just stopped buying the same intermediate work over and over.

At scale, dozens of workflows, hundreds of runs per day, prompt iteration, flaky providers, retry storms, that waste compounds fast.
...
Temporal is the grown-up in the room. If you need what it offers, nothing I'm building replaces that.

DriftQ is for the stage before you need Temporal. It's for small teams that want real durability guarantees without the operational tax of distributed infrastructure. It's for AI workloads where re-execution has a dollar cost and replay from a specific step actually saves you money.

If you outgrow it, that's fine. That's the plan.
